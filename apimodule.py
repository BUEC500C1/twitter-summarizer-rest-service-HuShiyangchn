...
def test_mytest():
    if keys exist:
        run actual twitter_api that goes on internet and fetches content
    else if keys do not exist:
        run fake stub twitter_api_test_stub function which returns hard coded results (tweets in json file in your repo)
'''
#def tweets():
tweetsnum = 10
...
for tweet in tweets:
    if (tweet.created_at.date() == datetime.date.today()):
        contentlist.append(tweet.full_text, encoding="utf-8")
        #fileobject.write('\n')
